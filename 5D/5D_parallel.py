...
@jit(nopython=True, parallel=True) # numba decorator. Significantly improves performance (~factor 100)
def MCintegral(alpha,dPhi,alpha1,alpha2, uniformAngles, distributionFunc):
    ...
    for i in prange(len(uniformAngles)): # xi is a 5D list here
        xi = uniformAngles[i]
...
def iterativeLL(par, var, pdf):
    ...
    for i in prange(len(var)): # iterate over samples
        v = var[i]
